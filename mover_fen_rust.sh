...
# Respeta MITTENS_LMR y MITTENS_HILOS del entorno. LMR queda activado por
# defecto; antes este script lo apagaba siempre y anulaba una mejora medida.
set -euo pipefail

FEN="${1:?uso: mover_fen_rust.sh 'FEN' [movetime_ms]}"
MOVETIME="${2:-5000}"
BIN="${MITTENS_BIN:-$HOME/mi-motor-rust/target/release/mittens}"
HILOS="${MITTENS_HILOS:-4}"
